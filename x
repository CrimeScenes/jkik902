...
local function UpdateFOV()
    ...
    local success, errorMsg = pcall(function()
        if Circle then
            Circle.Visible = shared.Forbidden['CamLock']['Normal']['Radius_Visibility']
            Circle.Radius = shared.Forbidden['CamLock']['Normal']['Radius']
...
local function GetTarget()
    if shared.Forbidden['Silent']['Startup']['mode'] == "target" then
        return TargetPlayer  -- Use the currently targeted player if in 'target' mode
    elseif shared.Forbidden['Silent']['Startup']['mode'] == "normal" then
...
Mouse.KeyDown:Connect(function(Key)
    if Key:lower() == shared.Forbidden['Target']['Keybind']:lower() then
        if shared.Forbidden['CamLock']['Normal']['Enabled'] then
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        local BodyPart
        if shared.Forbidden['CamLock']['Normal']['ClosestPart'] then
            BodyPart = GetClosestBodyPart(TargetPlayer.Character)
        else
            BodyPart = TargetPlayer.Character:FindFirstChild(shared.Forbidden['CamLock']['Normal']['HitPart'])
        end

        if BodyPart then
            local predictedPosition
            if shared.Forbidden['CamLock']['Normal']['Resolver'] then
                local humanoid = TargetPlayer.Character:FindFirstChildOfClass("Humanoid")
                if humanoid then
                    local moveDirection = humanoid.MoveDirection
                    predictedPosition = BodyPart.Position + (moveDirection * shared.Forbidden['CamLock']['Normal']['Prediction'])
                end
            else
                local targetVelocity = TargetPlayer.Character.HumanoidRootPart.Velocity
                predictedPosition = BodyPart.Position + (targetVelocity * shared.Forbidden['CamLock']['Normal']['Prediction'])
            end
            
            if predictedPosition then
                local DesiredCFrame = CFrame.new(Camera.CFrame.Position, predictedPosition)

                if shared.Forbidden['CamLock']['Normal']['SmoothnessEnabled'] then
                    Camera.CFrame = Camera.CFrame:Lerp(DesiredCFrame, shared.Forbidden['CamLock']['Normal']['Smoothness'])
                else
            ...
            if shared.Forbidden['Silent']['Startup']['Enabled'] and IsTargeting and TargetPlayer.Character:FindFirstChild("Humanoid") then
                if shared.Forbidden['Silent']['Startup']['mode'] == "target" then
                    ...
                    Replicated_Storage[RemoteEvent]:FireServer(Argument, closestPoint + velocity * shared.Forbidden['Silent']['Startup']['Prediction'])
